...
//如果int* p=arr;sizeof(p)=系统地址所占字节数；
//一维数组的元素就是一个一个数据对象，二维数据的元素是一个一个的一维数组；
//二维数组int array[3][4]:array可以视为一个数组指针；
// array[0]即二维数组的第一个元素，不就是一个一维数组吗；
//二维数组首地址array，一维数组首地址array[0]，第一个元素首地址&array[0][0]相同；

//对于一维数组arr[4]:
//一维数组名arr代表一维数组第一个元素arr[0]的地址，类型为int*
//对于二维数组type arr[3][4]:
//二维数组名arr代表二维数组第一个元素的地址，即第一行arr[0]的地址，类型为(int*) [4]，虽然数组名不等于指针，但是可以视为数组指针。
//arr+1是会移动四个元素；
//arr[0] + 1可以视为type*类型的指针，会移动一个元素，指向第一行的第二个元素的地址。arr + 1指向第二行的首地址。
//arr[0]实际上是一个一维数组名，自然就代表一维数组第一个元素的地址。
//总结：int arr[4]的元素是一个一个的int;int arr[3][4]的元素是一个一个的int[4]数组；

//常量指针：指向常量的指针，指针的值可以改变，但是指针指向的值无法改变；
//指针常量：指针的值不能改变，但指针指向的值一般来说可以改变，只要不是指向常变量；

//注意：虽然数组可以通过其名字隐式地转换为指向其首元素的指针，但数组本身并不是指针。
//数组和指针在内存布局、生命周期和可修改性上有显著的区别。
